...
import { UpdateAssignment } from '../models/update-assignment.model';
...
export class AssignmentApi {
  ...
  create(dto: CreateAssignment): Observable<Assignment> {
    return this.http
      .post<AssignmentResponseDto>(this.api, dto)
      .pipe(
        map(AssignmentMapper.toModel),
      );
  }
